...
class WebhookAdapter(BasePlatformAdapter):
        ...
        def _resolve(match: re.Match) -> str:
            ...
            for part in key.split("."):
                if isinstance(value, dict):
                    ...
                    # Failed to traverse into a non-dict — log and try
                    # fallback paths for well-known GitHub keys.
                    logger.warning(
                        "[webhook] template key %r not fully resolved "
                        "(stopped at %r, value type=%s); "
                        "top-level payload keys: %s",
                        key,
                        part,
                        type(value).__name__,
                        sorted(payload.keys())[:20],
                    )
                    resolved = self._try_fallback_keys(key, payload)
            ...
            if isinstance(value, str) and value.startswith("{") and value.endswith("}"):
                # The dict traversal returned the literal fallback template
                # because a key wasn't found — try fallback paths.
                logger.warning(
                    "[webhook] template key %r resolved to literal %r; "
                    "top-level payload keys: %s",
                    key,
                    value,
                    sorted(payload.keys())[:20],
    ...
    def _try_fallback_keys(
        self, key: str, payload: dict
    ) -> Optional[str]:
        """Try alternative payload paths when the primary key isn't found.

        GitHub webhook payloads can vary in structure across event types
        and delivery formats.  For example, ``repository.full_name`` is
        usually at the top level for ``pull_request`` events, but some
        delivery wrappers nest it differently.  This method checks known
        alternative locations so that delivery templates don't silently
        degrade to the literal ``{key}`` string.
        """
        _FALLBACK_PATHS: dict[str, list[list[str]]] = {
            "repository.full_name": [
                ["repository", "full_name"],                   # top-level (standard)
                ["pull_request", "base", "repo", "full_name"],  # PR base repo
                ["pull_request", "head", "repo", "full_name"],  # PR head repo
            ],
            "pull_request.number": [
                ["pull_request", "number"],                    # top-level (standard)
                ["number"],                                    # sometimes bare
            ],
            "pull_request.title": [
                ["pull_request", "title"],
            ],
        ...
        paths = _FALLBACK_PATHS.get(key)
        if not paths:
            # Generic fallback: try the first segment as a top-level key
            # and walk the rest inside it (handles cases where the payload
            # is already scoped to an event wrapper).
            parts = key.split(".")
            if len(parts) >= 2 and parts[0] in payload:
                paths = [parts]
            else:
                return None
        ...
        for path in paths:
            ...
            for part in path:
                ...
                if isinstance(value, str):
                    logger.info(
                        "[webhook] fallback path %s resolved %r → %r",
                        ".".join(path),
                        key,
                        value,
                    )
        ...
        # No fallback path succeeded — log the payload shape for debugging.
        logger.warning(
            "[webhook] all fallback paths failed for %r; "
            "top-level payload keys: %s",
            key,
            sorted(payload.keys())[:20],
        )
    ...
    async def _deliver_github_comment(
        ...
        # ── Guard against unresolved template literals ────────────
        # If template resolution failed upstream, the deliver_extra
        # values are literal {key} strings that gh CLI rejects.
        # Detect and reject early with a clear error.
        _TEMPLATE_RE = re.compile(r"^\{[a-zA-Z0-9_.]+\}$")
        for field_name, field_value in (("repo", repo), ("pr_number", str(pr_number))):
            if isinstance(field_value, str) and _TEMPLATE_RE.match(field_value):
                logger.error(
                    "[webhook] github_comment %s is an unresolved template "
                    "literal %r — template resolution failed. "
                    "deliver_extra: %s",
                    field_name,
                    field_value,
                    extra,
                ...
                return SendResult(
                    success=False,
                    error=(
                        f"Template resolution failed: {field_name} is "
                        f"still a template literal {field_value!r}"
                    ),
                )
